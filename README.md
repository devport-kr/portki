...
- **AI 에이전트 인터페이스**: AI가 코드베이스를 직접 읽고, 분석하여 위키 문서를 작성할 수 있도록 기계적인 파이프라인(스냅샷, 변경 감지, 검증, 영속화, 패키징 등)을 제공합니다.
- **청크 기반 생성 (Chunked Generation)**: 저장소의 규모가 클 경우, 문서를 여러 섹션으로 나누고 각 섹션 단위로 상세한 문서를 생성 및 검증하여 DB에 영속화(Persist)합니다.
...
| `persist-section` | 생성된 단일 섹션의 품질을 검증하고, 허용되면 PostgreSQL DB에 영속화합니다. |
| `finalize` | 작성 완료된 모든 섹션 간의 교차 검증을 수행하고, 최종 위키 초안 및 기준점을 업데이트합니다. |
| `package` | 위키 결과물 규격을 검증하고 배포용 `delivery.json`으로 패키징합니다. |
...
# 4. 각 섹션별 작성 및 영속화 (전체 섹션 완료 시까지 반복)
...
# (이후 영향을 받는 섹션만 다시 작성하여 persist-section 후 package 또는 finalize 수행)
```
...
devport-output/
  ...
  delivery/{owner}/{repo}/      # 패키징된 최종 배포용 데이터 (delivery.json)
